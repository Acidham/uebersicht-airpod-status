...
class AirPod(object):
    ...
    def __init__(self, ap: tuple):
        ...
        self.product = self.AIRPD_PRODUCT_INDX.get(self.product_id) if self.product_id in self.AIRPD_PRODUCT_INDX else "n/a"

    def get_paired_airpods(self) -> list:
        """
        ...
        for device in devices:
            for d_name, d_info in device.items():
                address: str = d_info.get('device_address')
                if d_info.get('device_connected') == "Yes" and 'device_productID' in d_info:
                    self.address = address
                    self.name = d_name
                    self.left = d_info.get("device_batteryLevelLeft")
                    self.right = d_info.get("device_batteryLevelRight")
                    self.case = d_info.get("device_batteryLevelCase")
                    self.product_id = int(d_info.get("device_productID"), 16)
                    self.vendor_id = int(d_info.get("device_vendorID"), 16)
                    self.product = self.AIRPD_PRODUCT_INDX.get(self.product_id) if self.product_id in self.AIRPD_PRODUCT_INDX else "n/a"
                    ...
                    if airpod.vendor_id == 76 and airpod.product_id in self.AIRPD_PRODUCT_INDX:
...
def get_device_html() -> list:
    ...
    """
    ap = AirPod()
    airpods: list = ap.get_paired_airpods()
    devices = list()
    for ap in airpods:
